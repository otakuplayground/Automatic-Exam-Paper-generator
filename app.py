...
class PreviousPaper(db.Model):  # New model for previous papers
    id = db.Column(db.Integer, primary_key=True)
    title = db.Column(db.String(150), nullable=False)
    filename = db.Column(db.String(150), nullable=False)
    uploaded_by = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)

class ExamPaper(db.Model):  # Model for generated exam papers
    id = db.Column(db.Integer, primary_key=True)
    title = db.Column(db.String(150), nullable=False)
...
def generate_exam_paper():
    ...
    subject = request.form['subject_exam']

    # Example logic for generating paper content
    questions = Question.query.filter_by(subject=subject).limit(10).all()
    content = "\n".join([f"{idx+1}. {q.question_text} [{q.marks} marks]" for idx, q in enumerate(questions)])

    # Save the generated paper
    exam_paper = ExamPaper(title=title, content=content, created_by=session['user_id'])
...
# View Generated Papers route
@app.route('/view_exam_papers', methods=['GET'])
...
# View a Single Exam Paper route
@app.route('/exam_paper/<int:paper_id>', methods=['GET'])
...
# Route to upload previous papers
@app.route('/upload_previous_paper', methods=['GET', 'POST'])
def upload_previous_paper():
    ...
    if request.method == 'POST':
        ...
        if file.filename == '':
            ...
            return redirect(url_for('upload_previous_paper'))
        ...
        if file:
            ...
            new_paper = PreviousPaper(title=title, filename=filename, uploaded_by=session['user_id'])
    ...
    return render_template('upload_previous_paper.html')

# Route to view previous papers
@app.route('/view_previous_papers', methods=['GET'])
def view_previous_papers():
    if 'user_id' not in session:
        return redirect(url_for('login'))

    papers = PreviousPaper.query.all()
    return render_template('view_previous_papers.html', papers=papers)
